...
def ask_model_judgments(args, client):
    ...
    with open(f"eos_defense_qs_1.txt", "r") as file:
        defendant_questions1=file.read()
    with open(f"eos_defense_qs_2.txt", "r") as file:
        defendant_questions2=file.read()
    with open(f"eos_prosecution_qs_1.txt", "r") as file:
        prosecution_questions1=file.read()
    with open(f"eos_prosecution_qs_2.txt", "r") as file:
        prosecution_questions2=file.read()
    defendant_questions=[defendant_questions1,defendant_questions2]
    prosecution_questions=[prosecution_questions1,prosecution_questions2]
    ...
    elicit_probability = "Please ***ONLY*** output a probability judgment which is an integer between 0 and 100, where 100 denotes certainty. This **MUST** appear at the end of your response and it should only be an integer nothing else!!!"

    # for loop for first set
        # append the evidence and the first question
    
    
    first_set_of_evidence_string="" 
    for evidence in first_set_of_evidence:
        first_set_of_evidence_string=first_set_of_evidence_string+evidence+"\n"
    second_set_of_evidence_string="" 
    for evidence in second_set_of_evidence:
        second_set_of_evidence_string=second_set_of_evidence_string+evidence+"\n"

    
    message_log.append({"role": "user", "content": first_set_of_evidence_string + first_set_of_questions[0] + "\n" + elicit_probability})
    ...
    message_log.append({"role": "user", "content": first_set_of_questions[1] + "\n" + elicit_probability})
    ...
    # append the evidence and the first question
    message_log.append({"role": "user", "content": second_set_of_evidence_string + second_set_of_questions[0] + "\n" + elicit_probability})
    # get the initial response
    model_response = chat_completion(args, client, message_log)
    while(is_integer(model_response.split()[-1])==False):
        model_response = chat_completion(args, client, message_log)
    ...
    message_log.append({"role": "user", "content": second_set_of_questions[1] + "\n" + elicit_probability})
    # and get the model's response
    model_response = chat_completion(args, client, message_log)
    while(is_integer(model_response.split()[-1])==False):
        model_response = chat_completion(args, client, message_log)
